defmodule EZProfiler.ProfilerOnTarget do
  ...
  def handle_event(:cast, {:code_start, pid, fun, label, display_label}, :waiting, %{profiler: profiler, profiler_node: profiler_node, profiling_start_wait_ref: sref,
    ...
    case do_profiling([pid], %{state | profiling_type_state: :code, code_profile_fun: fun, code_tracing_pid: pid}) do
      {:continue, new_state} -> {:next_state, :profiling, %{new_state | display_label: display_label, code_profile_fun: nil, current_label: label, cp_started: true,
      ...
      {_, new_state} -> {:stop, :normal, %{new_state | code_profile_fun: nil, profiling_start_wait_ref: nil, cp_started: true}}
    end
  end
  ...
  @doc false
  def handle_event({:call, from}, :code_stop, :profiling, %{cp_started: _, label_transition: label_transition?, current_labels: current_labels, profiler_node: profiler_node, current_results_filename: filename, code_manager_pid: cpid, profiler: profiler} = state) do
  ...
  @doc false
  def handle_event({:call, from}, :code_stop, any_state, %{display_labels: current_labels, code_manager_pid: cpid} =state) do
    respond_to_tester(state.test_pid, {:cstop2,state.cp_started, any_state, current_labels})
  ...
  defp set_next_state(%{display_labels: current_labels, label_transition?: true, code_manager_pid: cpid, code_tracing_pid: pid} = state) do
    respond_to_code(:code, :code_profiling_stopped, [pid])
    {_, next_state} = handle_event(:cast, {:allow_code_profiling, current_labels, pid, true}, :waiting, state)
    next_state
  end
